...
5. invariant (DIFFICULTY + RECIPE-RESISTANCE -- the strongest P1 predictor): is there an intermediate-state invariant (count / ordering / lifecycle / visibility) a naive implementation would violate while still passing value-equality checks? AND, critically: does this layer carry REPO/DOMAIN-DISCOVERABLE semantics -- existing conventions, protocol/standard rules, documented in-code contracts (e.g. SQL NULL-vs-empty, format-spec edge rules, framework lifecycle order) -- that hidden tests could FAIRLY pin WITHOUT the problem description stating them? That is where recipe-resistant difficulty lives: a feature whose hard edge rides on such semantics stays hard with a LEAN description, while a feature needing every detail spelled out in prose becomes a transcription recipe. Name the discoverable semantics you find (cite the file/symbol), or say none exist.
6. olympusViable (THE OVERALL VERDICT -- judge against the OLYMPUS VIABILITY STANDARD above): weighing ALL of the above (weight question 5's repo-discoverable semantics heavily -- without them, difficulty tends to collapse into either a spec-dense recipe description or unfair tests), can THIS repo host a feature that is genuinely HARD for a SOTA model AND genuinely VALUABLE AND one the maintainers would ACCEPT AND not already solved AND deterministically testable offline? Be skeptical: difficulty alone is NOT enough (a hard but thin or declined space fails -- the go-cloud/nestjs lessons), and value alone is NOT enough (valuable but easy fails). When unsure between yes and risky, choose "risky". Answer "yes" | "risky" | "no", naming the strongest realistic feature that is BOTH hard and acceptable (or why none exists).
