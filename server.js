...
5. invariant (DIFFICULTY + RECIPE-RESISTANCE): is there an intermediate-state invariant (count / ordering / lifecycle / visibility) a naive implementation would violate while still passing value-equality checks? IMPORTANT: you are reading ONE auto-picked subsystem, not the specific Olympus feature -- if you cannot pin a concrete invariant on THIS broad core, answer "unclear" (that is EXPECTED on a general engine core and does NOT mean the repo is easy; the real invariant is feature-lane-specific). AND: does this layer carry REPO/DOMAIN-DISCOVERABLE semantics -- existing conventions, protocol/standard rules, documented in-code contracts (e.g. SQL NULL-vs-empty, format-spec edge rules, framework lifecycle order) -- that hidden tests could FAIRLY pin WITHOUT the problem description stating them? That is where recipe-resistant difficulty lives: a feature whose hard edge rides on such semantics stays hard with a LEAN description, while a feature needing every detail spelled out in prose becomes a transcription recipe. Name the discoverable semantics you find (cite the file/symbol), or say none exist.
6. olympusViable (THE OVERALL VERDICT -- judge against the CURRENT standard): weigh THESE current-doctrine lenses, not just the three levers:
   - ADDITIVE-SOLVE RESISTANCE (heavy): can a substantial feature here be built by MODIFYING existing behavior threaded through existing code, or only by ADDING a new file/module beside an existing example? If the natural feature is additive (a new module next to siblings), it is EASY (a solver copies the template) -> risky. Modify-existing-behavior = hard.
   - SPEC-ENGINE GENRE (kill-switch): is this repo's PRIMARY VALUE conformance to a PUBLISHED STANDARD (a JS engine implementing ECMAScript, a regex engine, a spec-conformant parser/codec, a protocol impl)? If yes, its hardness IS the spec (algorithms a SOTA model recalls) and its novelty is host/config plumbing (additive) -> answer "no" (genre reject). NUANCE: an engine that merely USES a standard as an interface but whose hardness is INVENTED strategy the spec does not dictate (a SQL query PLANNER, a synthesis PASS, format-INTEGRITY, storage-CONSISTENCY) is NOT a spec-engine -- judge it on merits.
   - DO NOT downgrade to "risky" merely because the invariant lever (Q5) came back "unclear" on this generic subsystem -- that is expected; weigh helpers(none=hard) + approach-wrong + additive-resistance together.
   Then, weighing all of the above, can THIS repo host a feature that is genuinely HARD for a SOTA model AND genuinely VALUABLE AND one the maintainers would ACCEPT AND not already solved AND deterministically testable offline? Be skeptical: difficulty alone is NOT enough (a hard but thin or declined space fails -- the go-cloud/nestjs lessons), and value alone is NOT enough (valuable but easy fails). When unsure between yes and risky, choose "risky". Answer "yes" | "risky" | "no", naming the strongest realistic feature that is BOTH hard and acceptable (or why none exists).
